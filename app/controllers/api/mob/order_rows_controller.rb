class Api::Mob::OrderRowsController < Api::ApiApplicationControllerController
    ...
    if !client.nil?
      if payment.state == 'created'
        ...
        if payment.state == 'approved'
          ...
          redirect_to api_mob_order_rows_success_path(format: :html)
        else
          redirect_to api_mob_order_rows_not_authorized_path(format: :html)
        end
      else
        redirect_to api_mob_order_rows_unknown_path(format: :html)
      end
    end
  ...
  def cancel
    redirect_to api_mob_order_rows_canceled_path(format: :html)
  end
